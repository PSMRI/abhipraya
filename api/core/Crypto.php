...
class Crypto
{
    ...
    {
        $key = Env::get('ABHIPRAYA_FIELD_ENCRYPTION_KEY');

        /* Use a server-only secret for existing installations. Set the
         * dedicated variable in production to use an independently managed key. */
        if ($key === null) {
            $databaseSecret = Env::get('DB_PASSWORD');
            if ($databaseSecret === null) {
                throw new RuntimeException('Field encryption is not configured.');
            }
            $key = hash_hmac('sha256', 'abhipraya:profile-fields:v1', $databaseSecret);
        }
